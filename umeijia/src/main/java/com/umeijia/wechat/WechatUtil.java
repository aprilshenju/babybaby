...
public class WechatUtil {
    ...
    public static String obtainAccessToken() throws IOException {
        ...
        HttpGet httpGet = new HttpGet(wechatTokenUrl + "?grant_type=client_credential&appid=" + appId + "&secret=" +
                appSecret);
...
//                String expires_in = jsonObject.getString("expires_in");
     ...
     * @throws IOException
     */
    public static void sendTemplateMessage(String openId, String first, String name, String time, String location,
                                           String remark, int maxTimes) throws IOException {
        if (maxTimes <= 0) {
            return;
        }
        HttpClient httpClient = new DefaultHttpClient();
    ...
    public static boolean isEmail(String input) {
        Pattern pattern = Pattern.compile("^([a-z0-9A-Z]+[-|_|\\.]?)+[a-z0-9A-Z]@([a-z0-9A-Z]+(-[a-z0-9A-Z]+)?\\.)" +
                "+[a-zA-Z]{2,}$");
